...
def load_config():
    ...
    if (os.path.isfile(ini_file)):
        ...
        try:
            ...
            client_serial_port = config.get('Client', 'serial_port')
        except:
            return False
        try:
        ...
        return False
    return [server_credential, client_hostname, client_serial_port]
...
try:
    ...
    if (config == False):
        ...
        [server_credential, client_hostname, client_serial_port] = config

    # Create a components container
    components = Components()

    # Create a ZigBee interface
    zb_link = Zigbee_link(client_serial_port, 9600, components.callback)
